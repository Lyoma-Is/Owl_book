...
function getResultText(score, total) {
    if (score < 7) return `Ваша оценка: 2 <span style="padding: 5px;" class="bg_red">Не зачёт</span> <br><br> Количество баллов: ${score} из ${total}`;
    if (score < 15) return `Ваша оценка: 3 <span style="padding: 5px;" class="bg_yellow">Зачёт</span> <br><br> Количество баллов: ${score} из ${total}`;
    if (score < 22) return `Ваша оценка: 4 <span style="padding: 5px;" class="bg_green">Зачёт</span> <br><br> Количество баллов: ${score} из ${total}`;
...
function createVariants() {
    ...
    const variantsByYear = variants.reduce((acc, variant) => {
        const year = variant.year; // Добавлено значение по умолчанию
        if (!acc[year]) {
            acc[year] = [];
        }
    ...
    const createYearGroupHTML = (yearVariants) => {
        ...
        return reversedVariants.map((variant, i) => `
            <a id="variant${i+1}" 
               ...
               href="../../pages/mathOge/showTasksVars.html" 
...
async function loadVariants() {
    try {
        const response = await fetch('../../src/oge_math/showVariants.json');
