...
function createVariants() {
    ...
    block.innerHTML = variants.map((_, i) =>
        `<a id="variant${i+1}" class="vars-block__inner" href="../pages/variants/tasksOgeInf/showTasks.html" data-variant-index="${i}">Вариант ${i+1}</a>`
    ).join('');
...
function updateBreadcrumbWithVariant() {
    ...
    // Поиск индекса выбранного варианта (только после загрузки variants)
    const variantIndex = variants.findIndex(v => JSON.stringify(v) === JSON.stringify(selectedVariant));
    if (variantIndex !== -1) {
        breadcrumbLastLink.textContent = `Вариант ${variantIndex + 1}`;
    } else {
        breadcrumbLastLink.textContent = `Выбранный вариант`;
    }
}
document.addEventListener('DOMContentLoaded', async () => {
    await loadVariants(); // загружаем JSON

    updateBreadcrumbWithVariant(); // <-- обновляем хлебные крошки
