...
// };
// Где-то в основном коде приложения
async function loadTask(taskKey) {
  try {
    const url = tasks[taskKey];
    if (!url) return null;
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Ошибка загрузки ${taskKey}: ${response.status}`);
    }
    const taskData = await response.json();
    return taskData;
  } catch (error) {
    console.error(`Не удалось загрузить задание ${taskKey}:`, error);
    return null;
  }
}

// Пример использования для отображения задания 7
async function displayTaskSeven() {
  const taskData = await loadTask('seven');
  if (taskData) {
    // Вызываем вашу функцию для генерации HTML
    const taskHTML = generateTaskHTML('seven', taskData, false); // или true, если нужно показывать ответ
...
export default function generateTaskHTML(taskKey, item, tumbler = true) {
    ...
    const generateFooter = () => `</details>${span20()}<hr class="hr-between">${span20()}`;
    ...
    if (taskKey === 'one'){
      ...
      switch(typeTask){
         case 1: 
            ...
            answerBlock += `
                ${taskAn[3] === "м" ? `
                1) Один символ кодируется ${taskAn[1]} бит = ${taskAn[1]/8 < 2 ?  `${taskAn[1]/8} байт. ` : `${taskAn[1]/8} байта.`}${span10()}

                2) Вычеркнутое (Удалённое) слово занимает ${taskAn[2]} байта.${span10()}

                3) Количество символов в слове ${taskAn[2]}/${taskAn[1]/8} = ${taskAn[2]/(taskAn[1]/8)} символов.${span10()}
                ...
                taskAn[3] === "мм" ? `
                1) Один символ кодируется ${taskAn[1]} байтами. ${span10()}

                2) Вычеркнутое (Удалённое) слово занимает ${taskAn[2]} бит = ${taskAn[2]/8} байт.${span10()}

                3) Количество символов в слове ${taskAn[2]/8}/${taskAn[1]} = ${taskAn[2]/8/taskAn[1]} символов. ${span10()}
                ...
                2) Добавленное слово занимает ${taskAn[2]} байта.${span10()}
         ...
         case 2:
            ...
            answerBlock += `
                  ...
                  ${span10()}<p>3) Количество символов в слове ${taskAn[2]}/${taskAn[1]/8} = ${taskAn[2]/(taskAn[1]/8)} символов` : 
    ...
    if (taskKey === 'two'){
      ...
      switch(typeTask){
        case 1:
          answerBlock += beginTask();
          answerBlock +=`
            <p class="p-num"><b>${Nomer}.</b><em> № ${taskCounter}</em> ${tumbler ? generateAuthor() : ""} ${generateHard()}</p>${span10()}
            <p>${task1}</p>
        ...
        case 2:
          ...
          <p>${task1}</p>
           ${span20()}
            ...
            ${span10()}
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[0]}</p>${span10()}
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[1]}</p>${span10()}
        ...
        case 3:
          ...
          <p>${task1}</p>
            ...
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[0]}</p>${span10()}
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[1]}</p>${span10()}
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[2]}</p>${span10()}
            <p style="letter-spacing: 1px;">&emsp;&emsp;${task2[3]}</p>${span10()}
             <p>${task4}</p>
             ${span20()}`
          answerBlock += generateDate();
          answerBlock += generateHeader();
...
int main(){
   ...
   return 0; } </pre>
    ...
    if (taskKey === 'seven'){ 
      ...
      switch(typeTask){    
        case 1:
          answerBlock += beginTask();
          answerBlock += `<p class="p-num"><b>${Nomer}.</b><em> № ${taskCounter}</em> ${tumbler ? generateAuthor() : ""} ${generateHard()}</p> ${span10()}
 ...
 //   answerBlock += beginTask();
 //   answerBlock += beginTask(); answerBlock += endTask(); 
    if (taskKey === 'eight'){
      let Nomer = 8
      let k = task
      let r = ''
      function EightAn(t, v){
          if(t === 3){
             if (taskCounter === 196){
              r +=  `
              -------------------------------------------------------------------<br>
              ${task1[4]} = B = ${task2[4]}${span10()}
              ${task1[6]} = E = ${task2[6]}${span10()}
             ...
             if(taskCounter === 198){
              r +=  `
              -------------------------------------------------------------------<br>
              ${task1[5]} = E = ${task2[5]}${span10()}
              ${task1[6]} = D = ${task2[6]}${span10()}
              ${task1[7]} = B = ${task2[7]}${span10()}
              ${task1[4]} = ${task2[4]}<br>
              -------------------------------------------------------------------<br>
              ${task1[3]} = ?<br>
              -------------------------------------------------------------------<br>
              ${task1[2]} = D + E + F = ${task2[2]}${span10()}
              Из ${task1[0]} = A + B + D = ${task2[0]} -> A = 250 - B - D = 250 - 20 - 30 = 200${span10()}
              A = 200${span10()}
              Из ${task1[1]} = B + C + E = ${task2[1]} -> C = 200 - B - E = 200 - 20 - 60 = 120${span10()}
              C = 120${span10()}
              Отсюда A + B + C + D + E + F = 200 + 20 + 120 + 500 = 840${span10()}
              -------------------------------------------------------------------<br>
             ...
             if(taskCounter === 200){
              ...
              ${task1[3]} = ?<br>
              -------------------------------------------------------------------<br>
              ${task1[1]} = D + E + F = ${task2[1]}${span10()}
              
              Из ${task1[2]} = B + C + E = ${task2[2]} -> C = 450 - B - E = 450 - 50 - 40 = 360${span10()}
              C = 360${span10()}
              Из ${task1[4]} = A + B + D + E + F = ${task2[4]} -> A + B = 450 - (D + E + F) = 450 - 250 = 200${span10()}
              A + B = 200${span10()}
              Отсюда A + B + C + D + E + F = 200 + 360 + 250 = 810${span10()}
              -------------------------------------------------------------------<br>
             ...
             if(taskCounter === 201){
              ...
              -------------------------------------------------------------------<br>
              ${task1[2]} = D + E + F = ${task2[2]}${span10()}
              Из ${task1[0]} = A + B + D = ${task2[0]} -> A + B = 250 - D = 250 - 0 = 250${span10()}
              A + B = 250${span10()}
              Из ${task1[1]} = B + C + E = ${task2[1]} -> C = 200 - B - E = 200 - 20 - 10 = 170${span10()}
              C = 170${span10()}
              Отсюда A + B + C + D + E + F = 250 + 170 + 500 = 920${span10()}
              -------------------------------------------------------------------<br>
             ...
             if(taskCounter === 202){
              r +=  `
              -------------------------------------------------------------------<br>
              ${task1[4]} = B + G = ${task2[4]}${span10()}
              ${task1[5]} = E + G = ${task2[5]}<br> 
              -------------------------------------------------------------------<br>
              ${task1[3]} = G = ?<br>
              -------------------------------------------------------------------<br>
              Из ${task1[1]} = B + C + E + G = ${task2[1]} -> C + E = 200 - (B + G) = 200 - 200 = 0${span10()}
              C + E = 0 -> C = 0, E = 0${span10()}
              Из ${task1[5]} = E + G = ${task2[5]} -> G = 60 - 0 ${span10()}
              G = 60${span10()}
              -------------------------------------------------------------------<br>
          ...
          if (t === 2){
            if (v === 1){
              r += `<b>${task1[6]}</b> = ${task1[0]} + ${task1[1]} + ${task1[2]} - ${task1[3]} - ${task1[4]} - ${task1[5]}${span10()}
              <b>${task1[6]}</b> = ${task2[0]} + ${task2[1]} + ${task2[2]} - ${task2[3]} - ${task2[4]} - ${task2[5]}${span10()}
              <b>${task1[6]}</b> = ${task2[0] + task2[1] + task2[2] - task2[3] - task2[4] - task2[5]} `  
            }
            if (v === 2){
              r += `<b>${task1[6]}</b> = ${task1[2]} + ${task1[3]} + ${task1[4]} + ${task1[5]} - ${task1[0]} - ${task1[1]}${span10()}
              <b>${task1[6]}</b> = ${task2[2]} + ${task2[3]} + ${task2[4]} + ${task2[5]} - ${task2[0]} - ${task2[1]}${span10()}
      ...
      switch(typeTask){       
        case 1: 
          ...
          answerBlock += generateHeader();        
          answerBlock += `
            N<sub>A&B</sub> + N<sub>A|B</sub> = N<sub>A</sub> + N<sub>B</sub>${span10()}
            ${task2[3] === 1 ? `
            ${
              k === "&" ? `
        ...
        case 3:
          ...
          answerBlock += `
                      ...
                      ${ task2[3] === 4 
                        ...
                        ${span10()}
                        ${task1[0]} = A + B + D + G = ${task2[0]}${span10()}
                        ${task1[1]} = B + C + E + G = ${task2[1]}${span10()}
                        ...
                        ${span10()}
                        ${task1[0]} = A + B + D = ${task2[0]}${span10()}
                        ${task1[1]} = B + C + E = ${task2[1]}${span10()}
      ...
      switch(typeTask){
        case 1:
          ...
          Используя информацию и иллюстративный материал, содержащийся в каталоге <b>${task1[0]}</b>, создайте презентацию из трёх слайдов на тему <b>«${task1[1]}»</b>.<br>
           ...
           ${task3}<br>${span10()}
          <details class="open-del"><summary >Развернуть описание</summary>${span10()}
          ...
          ${span20()}
          <b>Требования к оформлению работы</b>${span10()}
          1. Ровно три слайда без анимации. Параметры страницы (слайда): экран (16:9), ориентация альбомная.${span10()}
          2. Содержание, структура, форматирование шрифта и размещение изображений на слайдах:${span10()}
          ● первый слайд – титульный слайд с названием презентации, в подзаголовке титульного слайда в качестве информации об авторе презентации указывается идентификационный номер участника экзамена;${span10()}
          ● второй слайд – основная информация в соответствии с заданием, размещённая по образцу на рисунке макета слайда 2:${span10()}
          &emsp;• заголовок слайда;${span10()}
          &emsp;• два изображения;${span10()}
          &emsp;• два блока текста;${span10()}
          ● третий слайд – дополнительная информация по теме презентации, размещённая по образцу на рисунке макета слайда 3:${span10()}
          &emsp;• заголовок слайда;${span10()}
          &emsp;• три изображения;${span10()}
          &emsp;• три блока текста.${span10()}
          На макетах слайдов существенным является наличие всех объектов, включая заголовки, их взаимное расположение. 
          Выравнивание объектов, ориентация изображений выполняются произвольно в соответствии с замыслом автора работы и служат наилучшему раскрытию темы.</details>${span20()}
          <table class="table_2_3">
            <tbody>
              <tr >
          ...
          ${span20()}
          В презентации должен использоваться единый тип шрифта (рубленый, с засечками или моноширинный).${span10()}
          Размер шрифта для названия презентации на титульном слайде –${span10()}
          40 пунктов, для подзаголовка на титульном слайде и заголовков слайдов –${span10()}
          24 пункта, для подзаголовков на втором и третьем слайдах и для основного текста – 20 пунктов.${span10()}
    ...
    if (taskKey === 'thirteentwo'){
      ...
      switch(typeTask){
        case 1:
          answerBlock += beginTask();
          answerBlock += `<p class="p-num"><b>${Nomer}.</b><em> № ${taskCounter}</em> ${tumbler ? generateAuthor() : ""} ${generateHard()}</p> ${span10()}
          Создайте в текстовом редакторе документ и напишите в нём следующий текст, точно воспроизведя всё оформление текста, имеющееся в образце.${span10()}
          Данный текст должен быть набран шрифтом размером 14 пунктов обычного начертания. Отступ первой строки абзацев основного текста – 1 см.${span10()}
          Расстояние между строками текста не менее одинарного, но не более полуторного междустрочного интервала.${span10()}
          ...
          Таблица выровнена на странице по центру горизонтали. Ширина таблицы меньше ширины основного текста.${span10()}
          ...
          Интервал между заголовком текста и текстом, между абзацами текста, между текстом и таблицей не менее 12 пунктов, но не более 24 пунктов.${span10()}
          Текст сохраните в файле, имя которого Вам сообщат организаторы.<br>
          Файл ответа необходимо сохранить в одном из следующих форматов: *.odt, или *.doc, или <a class="print" style="text-decoration: none; color: black;" href="../../../src/inf_file/task13/13.2.docx">*.docx</a>.
          <br><em>${task4}</em>${span10()}
          <hr>
          
          <img class="img-13_2" src="../../../img/task13.2/task13.2_${taskCounter}.png"${span20()}
          `
          answerBlock += generateDate();
          answerBlock += `<hr class="hr-between">${span20()}`
          if(tumbler === false){
...
${span10()}
Каждая строка таблицы содержит запись об одной перевозке.${span10()}
...
Всего в электронную таблицу были занесены данные по 1000 учащихся. Порядок записей в таблице произвольный.         
      ...
      switch(typeTask){
        case 1:
          ...
          На основании данных, содержащихся в этой таблице, выполните задания. ${span10()}  
   
          ${task4}${span10()} 
          ${task5}${span10()}          
          ${task6}${span10()} 

          Полученную таблицу необходимо сохранить под именем, указанным организаторами экзамена.${span10()}
          <div class="download">  
    ...
    if (taskKey === 'fifteen'){
      ...
      switch(typeTask){
        case 1:
          ...
          ${span20()}
          `
          answerBlock += `<b><em>Выполните задание.</em></b>${span10()}
          ${task1}<br>
          ...
          ${taskCounter === 950 || taskCounter === 1457 || taskCounter === 1499 || taskCounter === 1515 ? 
            `
          <img class="img-14" style="width: 15%;" src="../../../img/task15/task15_${taskCounter}_1.png">
          ${span10()}
          ${task2}
          ${span10()}

          <img class="img-14" style="width: 15%;" src="../../../img/task15/task15_${taskCounter}_2.png">
          ${span10()}
          ${task3}
          ${span10()}
            ` 
            :
            `
          <img class="img-14" src="../../../img/task15/task15_${taskCounter}_1.png">
          ${span10()}
          ${task2}
          ${span10()}

          <img class="img-14" src="../../../img/task15/task15_${taskCounter}_2.png">
          ${span10()}
          ${task3}
          ${span10()}
          `
    ...
    if (taskKey === 'sixteen'){
      ...
      switch(typeTask){
        case 1:
          answerBlock += beginTask();
          answerBlock += `<p class="p-num"><b>${Nomer}.</b><em> № ${taskCounter}</em> ${tumbler ? generateAuthor() : ""} ${generateHard()}</p> ${span10()}
          Дайте развернутый ответ.${span10()}
          ${task1}${span10()}
          ${task2}${span10()}
          ${task3}${span10()}
          <em>Пример работы программы</em>${span10()}
